...
function no_comment__activate() {
	...
	set_transient( 'no_comment_plugin_activation', $message, 10 );
}
...
function no_comment__get_activation_message() {
	...
	$message .= sprintf( '<li>%s</li>', __( 'Closed comments on all published posts of all post types.', 'no-comment' ) );
	$message .= sprintf( '<li>%s</li>', __( 'Set default comment status to closed.', 'no-comment' ) );
